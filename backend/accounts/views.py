...
class SendOTPView(APIView):
    ...
    def post(self, request):
        ...
        # send OTP via email -- for now just log so it appears even when stdout isn't shown
        logger = logging.getLogger(__name__)
        logger.info(f"[OTP for {validated['email']}] {otp}")
        return Response({"message": "OTP sent (check logs/console)"}, status=status.HTTP_200_OK)
