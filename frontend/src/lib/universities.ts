...
function loadRejected(): Array<
  UniversityVerification & { reason?: string; rejectedAt?: number }
> {
  try {
...
function saveApproved(m: Record<string, { uniId: string; name?: string }>) {
  try {
    localStorage.setItem(APPROVED_KEY, JSON.stringify(m));
  } catch (e) {}
}
...
// persisted stores
export const approvedUniversities: Record<
  string,
  { uniId: string; name?: string }
> = loadApproved();
export const pendingUniversityVerifications: UniversityVerification[] =
  loadPending();
export const universityPasswords: Record<string, string> = loadPasswords();

export const rejectedUniversities: Array<
  UniversityVerification & { reason?: string; rejectedAt?: number }
> = loadRejected();

export const registeredUniversities: Record<string, RegisteredUniversity> = {};

export async function fetchApprovedUniversities(): Promise<
  Array<{ uniId: string; name?: string; email?: string; address?: string }>
> {
...
export function registerUniversity(u: RegisteredUniversity) {
  ...
  const exists =
    registeredUniversities[u.uniId] ||
    approvedUniversities[u.uniId] ||
    pendingUniversityVerifications.find((p) => p.uniId === u.uniId);
...
export function approveUniversityVerification(uniId: string) {
  const idx = pendingUniversityVerifications.findIndex(
    (p) => p.uniId === uniId,
  );
